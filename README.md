...
5. **Upload Gambar**: Fitur opsional untuk mengunggah gambar sampul berita lengkap dengan fitur _Preview Image_ instan.
...
3. **Salin File Environment**
   Buat file `.env` dari salinan `.env.example`:
    ...
    _(Jika menggunakan Windows Command Prompt, gunakan `copy .env.example .env`)_
...
5. **Konfigurasi Database**
   Buka file `.env`, lalu atur bagian database sesuai dengan sistem komputermu. Jika kamu menggunakan MySQL (XAMPP/Laragon):
    ...
    _Pastikan kamu sudah membuat database kosong bernama `portalberita` di phpMyAdmin/MySQL sebelum lanjut._
...
6. **Jalankan Migrasi & Seeder Database**
   Perintah ini akan membuat tabel di database dan mengisi Kategori bawaan:
    ...
    ```
    _Perintah seeder ini akan otomatis membuatkan akun Admin bawaan untukmu:_
