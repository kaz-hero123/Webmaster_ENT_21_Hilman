...
<body class="bg-gray-50 text-gray-800 font-sans antialiased min-h-screen">
    ...
    <main class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 sm:p-10">
            <h1 class="text-3xl font-bold text-gray-900 mb-8">Edit Berita</h1>
            ...
            <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') ?? $post->title }}" required class="w-full rounded-md border border-gray-300 py-2 px-3 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 text-lg">
                </div>

                <div>
                    <label for="content" class="block text-sm font-medium text-gray-700 mb-1">Isi Konten <span class="text-red-500">*</span></label>
                    <textarea name="content" id="content" rows="10" required class="w-full rounded-md border border-gray-300 py-2 px-3 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">{{ old('content') ?? $post->content }}</textarea>
                </div>
                ...
                <div>
                    ...
                    @if($post->image)
                        <div class="mb-4 relative inline-block">
                            <span class="block text-xs font-medium text-gray-500 mb-1">Gambar Saat Ini:</span>
                            <img src="{{ asset('storage/' . $post->image) }}" alt="Gambar" class="h-32 w-auto object-cover rounded-md border border-gray-200">
                        </div>
                    @endif
